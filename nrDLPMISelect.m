function [PMISet,info] = nrDLPMISelect(carrier,csirs,reportConfig,nLayers,H,varargin)
%nrDLPMISelect PDSCH precoding matrix indicator calculation
%   [PMISET,INFO] = nrDLPMISelect(CARRIER,CSIRS,REPORTCONFIG,NLAYERS,H)
...
%
%   [PMISET,INFO] = nrDLPMISelect(...,NVAR) specifies the estimated noise
...
%   Copyright 2020-2024 The MathWorks, Inc.
    ...
    subbandInfo = nr5g.internal.nrDLPMISubbandInfo(carrier,reportConfig);
...
%   CSIRSIND and noise variance NVAR.
    ...
    if ~isscalar(unique(csirs.NumCSIRSPorts))
        error('nr5g:nrDLPMISelect:InvalidCSIRSPorts',...
    ...
    if ~all(strcmpi(cdmType,cdmType{1}))
        error('nr5g:nrDLPMISelect:InvalidCSIRSCDMTypes',...
    ...
    if ~isfield(reportConfig,'NSizeBWP')
        error('nr5g:nrDLPMISelect:NSizeBWPMissing','NSizeBWP field is mandatory.');
    end
    ...
    if ~isfield(reportConfig,'NStartBWP')
        error('nr5g:nrDLPMISelect:NStartBWPMissing','NStartBWP field is mandatory.');
    end
    ...
    if nStartBWP < carrier.NStartGrid
        error('nr5g:nrDLPMISelect:InvalidNStartBWP',...
    ...
    if (nSizeBWP + nStartBWP)>(carrier.NStartGrid + carrier.NSizeGrid)
        error('nr5g:nrDLPMISelect:InvalidBWPLimits',['The sum of starting resource '...
    ...
    if ~isType1MultiPanel
        if NumCSIRSPorts > 2
            ...
            if isempty(configIdx)
                error('nr5g:nrDLPMISelect:InvalidPanelConfiguration',['The given panel configuration ['...
            ...
            if isType2 || isEnhType2
                error('nr5g:nrDLPMISelect:InvalidCSIRSPortsConfigurationForType2',...
                    ['The minimum required number of CSI-RS ports for ' codebookType ' codebooks is 4.']);
            end
        end
    else
        if ~any(NumCSIRSPorts == [8 16 32])
            error('nr5g:nrDLPMISelect:InvalidNumCSIRSPortsForMultiPanel',['For' ...
        ...
        if Pcsirs ~= NumCSIRSPorts
            error('nr5g:nrDLPMISelect:InvalidMultiPanelDimensions',...
                ['For the configured number of CSI-RS ports (' num2str(NumCSIRSPorts)...
        ...
        if isempty(configIdx)
            error('nr5g:nrDLPMISelect:InvalidMultiPanelConfiguration',['The given panel configuration ['...
        ...
        if reportConfig.CodebookMode == 2 && Ng ~= 2
            error('nr5g:nrDLPMISelect:InvalidNumPanelsforGivenCodebookMode',['For' ...
    ...
    if isfield(reportConfig,'PRGSize') && isType1SinglePanel
        ...
        if ~(isempty(reportConfig.PRGSize) || any(reportConfig.PRGSize == [2 4]))
            error('nr5g:nrDLPMISelect:InvalidPRGSize',...
    ...
    if strcmpi(reportConfig.PMIMode,'Subband') && isempty(reportConfig.PRGSize)
        if nSizeBWP >= 24
            if ~isfield(reportConfig,'SubbandSize')
                error('nr5g:nrDLPMISelect:SubbandSizeMissing',...
            ...
            if ~any(NSBPRB == validNSBPRBValues)
                error('nr5g:nrDLPMISelect:InvalidSubbandSize',['For the configured BWP size (' num2str(nSizeBWP) ...
    ...
    if isType2
        ...
        if isfield(reportConfig,'PhaseAlphabetSize')
            if ~any(reportConfig.PhaseAlphabetSize == [4 8])
                error('nr5g:nrDLPMISelect:InvalidPhaseAlphabetSize',...
    ...
    elseif isEnhType2
        ...
        if NumCSIRSPorts == 4
            if reportConfig.ParameterCombination >= 3
                error('nr5g:nrDLPMISelect:InvalidParameterCombination',...
        ...
        if isfield(reportConfig,'NumberOfPMISubbandsPerCQISubband')
            if ~any(reportConfig.NumberOfPMISubbandsPerCQISubband == [1 2])
                error('nr5g:nrDLPMISelect:InvalidNumberOfPMISubbandsPerCQISubband',...
    ...
    if ~isempty(csirsInd)
        ...
        if nLayers > maxPossibleNLayers
            error('nr5g:nrDLPMISelect:InvalidNumLayers',...
        ...
        for q1Val = 1:O1
            for q2Val = 1:O2
                ...
                if ~(any(isnan(WTemp),'all') || any(isinf(WTemp),'all'))
                    SINRValues(:,:,i12Val,q1Val,q2Val) = nr5g.internal.nrPrecodedSINR(H,nVar,WTemp);
                end
            end
        end
    end
...
% MAXALLOWBLEAMP, defined from codebook subset restriction
    ...
    stdAmps = [0 0.125 0.1768 0.25 0.3536 0.5 0.7071 1];
