    ...
    def _sample_mcs(self):
        if self.mcs_spread == 0:
            ...
            jitter = torch.randint(
                        low=-self.mcs_spread,
                        high=self.mcs_spread + 1,
                        size=(self.n_ue,),
                        generator=self._gen,
                        device=self.device,
                        dtype=torch.long
                    )
            mcs = torch.clamp(self.mcs_mean + jitter, min=0, max=self.max_mcs)

        self._curr_mcs = torch.tensor(mcs, dtype=int)

        jitter = torch.randint(
                low=-self.mcs_spread,
                high=self.mcs_spread + 1,
                size=(self.n_ue,self.n_rbg),
                generator=self._gen,
                device=self.device,
                dtype=torch.long
            )

        self.curr_mcs_subband = torch.clamp(self._curr_mcs.reshape(-1,1) + jitter, min=0, max=self.max_mcs)
