function [RI,PMISet,PMIInfo] = nrRISelect(carrier,csirs,reportConfig,H,varargin)
% nrRISelect Rank indicator calculation
%   [RI,PMISET,PMIINFO] = nrRISelect(CARRIER,CSIRS,REPORTCONFIG,H) returns
%   the downlink channel rank indicator (RI) value RI and corresponding
...
%   configuration REPORTCONFIG, and estimated channel information H. The
%   function also returns the additional information about the signal to
%   interference and noise ratio (SINR) values that are used for the CQI
%   computation and PMI computation.
%   
...
%
%   [RI,PMISET,PMIINFO] = nrRISelect(CARRIER,CSIRS,REPORTCONFIG,H,NVAR) specifies
...
%   not given as input.
%
%   [RI,PMISET,PMIINFO] = nrRISelect(...,NVAR,ALG) also specifies the algorithm ALG
...
%   [RI,PMISet] = nrRISelect(carrier,csirs,reportConfig,H,nVar)
...
%   Copyright 2021-2023 The MathWorks, Inc.
    ...
    % given configuration
    PMISubbandInfo = nr5g.internal.nrDLPMISubbandInfo(carrier,reportConfig);
...
function [RI,PMISet,PMIInfoOut] = riSelectPMI(carrier,csirs,reportConfig,H,nVar,validRanks,PMISubbandInfo)
    ...
    for rankIdx = validRanks
        % PMI selection
        [PMI,PMIInfo] = nr5g.internal.nrDLPMISelect(carrier,csirs,reportConfig,rankIdx,H,nVar);
        ...
        if ~all(isnan(PMI.i1))
            ...
            PMIInfoOut = PMIInfo; %assign Nan PMIInfo as an output
        end
...
function [RI,PMISet,PMIInfo] = riSelectCQI(carrier,csirs,reportConfig,H,nVar,validRanks,PMISubbandInfo)
    ...
    for rank = validRanks
        ...
        [cqi,pmi(rank),cqiInfo,pmiInfo(rank)] = nr5g.internal.nrCQISelect(carrier,csirs,reportConfig,rank,H,nVar); %#ok<AGROW>
        ...
        if all(cqiWideband ~= 0)
            if ~any(isnan(cqiWideband))
                ...
                mcs = nr5g.internal.nrCQITables(reportConfig.CQITable,cqiWideband);
...
%   CSIRSIND, the noise variance NVAR and algorithm ALG.
    ...
    if ~isscalar(unique(csirs.NumCSIRSPorts))
        error('nr5g:nrRISelect:InvalidCSIRSPorts',...
    ...
    if ~all(strcmpi(cdmType,cdmType{1}))
        error('nr5g:nrRISelect:InvalidCSIRSCDMTypes',...
    ...
    if ~isfield(reportConfig,'NSizeBWP')
        error('nr5g:nrRISelect:NSizeBWPMissing','NSizeBWP field is mandatory.');
    end
    ...
    if ~isfield(reportConfig,'NStartBWP')
        error('nr5g:nrRISelect:NStartBWPMissing','NStartBWP field is mandatory.');
    end
    ...
    if nStartBWP < carrier.NStartGrid
        error('nr5g:nrRISelect:InvalidNStartBWP',...
    ...
    if (nSizeBWP + nStartBWP)>(carrier.NStartGrid + carrier.NSizeGrid)
        error('nr5g:nrRISelect:InvalidBWPLimits',['The sum of starting resource '...
    ...
    if strcmpi(reportConfig.PMIMode,'Subband') || (maxSEFlag && strcmpi(reportConfig.CQIMode,'Subband'))
        if nSizeBWP >= 24
            if ~isfield(reportConfig,'SubbandSize')
                error('nr5g:nrRISelect:SubbandSizeMissing',...
            ...
            if ~any(NSBPRB == validNSBPRBValues)
                error('nr5g:nrRISelect:InvalidSubbandSize',['For the configured BWP size (' num2str(nSizeBWP) ...
