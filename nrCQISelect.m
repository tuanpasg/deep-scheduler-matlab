function [CQI,PMISet,CQIInfo,PMIInfo] = nrCQISelect(carrier,varargin)
% nrCQISelect PDSCH Channel quality indicator calculation
%   [CQI,PMISET,CQIINFO,PMIINFO] = nrCQISelect(CARRIER,CSIRS,REPORTCONFIG,NLAYERS,H)
...
%   <a href="matlab:help('nr5g.internal.nrDLPMISelect')">nrDLPMISelect</a> function.
%
...
%
%   [CQI,PMISET,CQIINFO,PMIINFO] = nrCQISelect(CARRIER,CSIRS,REPORTCONFIG,NLAYERS,H,NVAR)
...
%
%   [CQI,PMISET,CQIINFO,PMIINFO] = nrCQISelect(CARRIER,CSIRS,REPORTCONFIG,NLAYERS,H,NVAR,SINRTABLE)
...
%
%   [CQI,PMISET,CQIINFO,PMIINFO] = nrCQISelect(CARRIER,BWP,CQICONFIG,CSIRSIND,H,NVAR) 
...
%   [CQI,PMISet,CQIInfo,PMIInfo] = nr5g.internal.nrCQISelect(carrier,csirs,reportConfig,numLayers,H,nVar)
...
%   Copyright 2019-2024 The MathWorks, Inc.
    ...
    if ~isCSIRSObjSyntax
        ...
        if isempty(csirsIndSubs_k) || (nVar == 0)
            ...
            for reIdx = 1: numel(csirsIndSubs_k)
                ...
                % CSI-RS is present
                SINRsperRE(reIdx) = nr5g.internal.nrPrecodedSINR(Htemp,nVar,W);
            end
    ...
    if ~inputSINRTable
        ...
        [pdsch,pdschExt] = nrCSIReferenceResource(carrier,reportConfig,nLayers);
...
%   SINRTABLE for SINR to CQI mapping, and the noise variance NVAR.
    ...
    if ~isfield(reportConfig,'NSizeBWP')
        error('nr5g:nrCQISelect:NSizeBWPMissing','NSizeBWP field is mandatory.');
    end
    ...
    if ~isfield(reportConfig,'NStartBWP')
        error('nr5g:nrCQISelect:NStartBWPMissing','NStartBWP field is mandatory.');
    end
    ...
    if nStartBWP < carrier.NStartGrid
        error('nr5g:nrCQISelect:InvalidNStartBWP',...
    ...
    if (nSizeBWP + nStartBWP)>(carrier.NStartGrid + carrier.NSizeGrid)
        error('nr5g:nrCQISelect:InvalidBWPLimits',['The sum of starting resource '...
    ...
    if isfield(reportConfig,'PRGSize') && strcmpi(reportConfig.CodebookType,'Type1SinglePanel')
        ...
        if ~(isempty(reportConfig.PRGSize) || any(reportConfig.PRGSize == [2 4]))
            error('nr5g:nrCQISelect:InvalidPRGSize',...
    ...
    if strcmpi(reportConfig.CQIMode,'Subband') ||...
        ...
        if nSizeBWP >= 24
            ...
            if ~isfield(reportConfig,'SubbandSize')
                error('nr5g:nrCQISelect:SubbandSizeMissing',...
            ...
            if ~any(NSBPRB == validNSBPRBValues)
                error('nr5g:nrCQISelect:InvalidSubbandSize',['For the configured BWP size (' num2str(nSizeBWP) ...
 ...
 validateattributes(nLayers,{'numeric'},{'scalar','integer','positive','<=',maxNLayers},fcnName,['NLAYERS(' num2str(nLayers) ') for ' codebookType ' codebooks']);
    ...
    if ~isempty(csirsInd)
        ...
        if nLayers > maxNLayers
            error('nr5g:nrCQISelect:InvalidNumLayers',...
...
%   NVAR         - Estimated noise variance
    ...
    validateattributes(carrier,{'nrCarrierConfig'},{'scalar'},'nrCQISelect','CARRIER');
    ...
    if isstruct(variableInputArgs{1})
        ...
        validateattributes(csirsInd,{'numeric'},{'positive','integer'},'nrCQISelect','CSIRSIND');
    ...
    elseif isa(variableInputArgs{1},'nrCSIRSConfig')
        ...
        % 5.2.2.3.1
        validateattributes(csirs,{'nrCSIRSConfig'},{'scalar'},'nrCQISelect','CSIRS');
        if ~isscalar(unique(csirs.NumCSIRSPorts))
            error('nr5g:nrCQISelect:InvalidCSIRSPorts','All the CSI-RS resources must be configured to have same number of CSI-RS ports.');
        else
        ...
        if ~all(strcmpi(cdmType,cdmType{1}))
            error('nr5g:nrCQISelect:InvalidCSIRSCDMTypes','All the CSI-RS resources must be configured to have same CDM lengths.');
        end
        ...
        error('nr5g:nrCQISelect:InvalidInputsToTheHelper','The second input argument can be either a structure or a CSI-RS configuration object.');
    end
...
function [pdsch,pdschExt] = nrCSIReferenceResource(carrier,reportConfig,numLayers)
    ...
    % Set DMRSLength to 2, to support upto 8 PDSCH layers
