%nrMaxPUSCHPrecodingMatrixIndicator maximum PUSCH PMI
%   W = nrMaxPUSCHPrecodingMatrixIndicator (NLAYERS,NPORTS) returns the
...
function maxTPMI = nrMaxPUSCHPrecodingMatrixIndicator(nlayers,nports)
   
    narginchk(2,2);
    
    fcnName = 'nrMaxPUSCHPrecodingMatrixIndicator';
    ...
    if ~any(nports==[1 2 4])
        error('nr5g:nrMaxPUSCHPrecodingMatrixIndicator:InvalidNPorts','Invalid number of ports (%d). The number of ports must be 1, 2 or 4. ', nports);
    elseif nlayers>nports
        error('nr5g:nrMaxPUSCHPrecodingMatrixIndicator:TooManyLayers','The number of layers (%d) must be lower than or equal to the number of ports (%d).',nlayers, nports);
    end
